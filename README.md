# AI 活动雷达 (ai-activity-radar)

> 在时间截止前找到 AI 机会

**在线地址：** https://hope0719.github.io/ai-activity-radar/
...
2. **[JS-banana/ai-opportunity-radar](https://github.com/JS-banana/ai-opportunity-radar)**（即 airadar.laifuyou.com 的开源源码）
   本项目的**布局与数据架构参考**。我们参考了它的页面视觉（米色纸感、卡片网格、即将截止高亮区）与数据模型（飞书多维表格 → 快照 JSON → 静态托管），并直接复用了其公开的 `snapshot.json` 作为**第一数据源**（161 条字段完整的活动记录），同时在其基础上新增了中文映射、第二数据源（LucianaiB 飞书表）合并、去重与本地化增强。

---

## 一、项目目的

**用一张随时更新的「AI 机会雷达图」，帮开发者/创作者在截止日期前发现值得投入的 AI 活动（黑客松、竞赛、开发挑战、权益福利等）。**

核心问题：AI 领域的比赛/活动信息分散在 Devpost、ModelScope、天池、GitCode、微信公众号等大量渠道，且**高度依赖时效**（过期即失效）。本项目把散落的机会聚合到单一页面，并持续自动同步，解决三个痛点：

1. **信息聚合**：将多源 AI 活动统一为结构化数据，按类型/地区/排序筛选，一眼找到合适的。
2. **时效管理**：高亮"即将截止"活动，动态隐藏过期超过 7 天的活动（后端保留全量，前端按时效过滤）。
3. **投入判断**：每张卡片提供推荐指数、难度、奖励类型、官方确认状态、行动建议（立即行动/值得参加/先关注/跳过），帮用户决定"要不要参加"。

数据当前规模：**178 条活动**（黑客松 73 / AI 竞赛 49 / 开发挑战 32 / 内容创作 10 / 权益福利 8 / 开发激励 6），覆盖全球（110）与中国（53）为主。
...
│       src/data/snapshot.json（字段完整, 161 条）               │
...
┌─────────────────────────────────────────────────────────────┐
│                    GitHub Pages 静态托管                      │
│   index.html（单页应用, 无后端, data.json 驱动渲染）            │
│   https://hope0719.github.io/ai-activity-radar/              │
└─────────────────────────────────────────────────────────────┘
```
...
| 层 | 技术 | 说明 |
|---|---|---|
| 前端 | 原生 HTML + CSS + JavaScript（ES6+） | 单文件 `index.html`，无框架、无构建、无后端 |
| 数据 | `data.json`（JSON） | 全量活动数据，前端 fetch 拉取后渲染 |
...
```json
{
  ...
  "updated_at": "2026-08-23T16:08:33.132Z",
...
| `startAt` / `endAt` | ISO datetime | 开始/截止时间（endAt 为时效判定主依据） |
...
### 2.5 页面渲染规则（index.html）

- **数据加载**：fetch `data.json`，无构建、无框架。
- **时效过滤**：`endAt` 为空 → 长期开放（一直展示）；`endAt` 超过当前时间 7 天 → 从列表隐藏（后端数据全量保留，前端动态过滤）。
- **即将截止区**：取未结束且 deadline 最近的 4 条，红/琥珀色徽章标注剩余天数。
- **卡片信息**：标题（自动去金额）→ 厂商·地区 → 截止日 + 官方状态徽章 + 行动建议 + 奖励类型 chips → 推荐★ + 难度★ 同行 → 奖励详情盒 → 参与方式摘要 → 类型标签。
- **筛选排序**：类型 / 地区 / 排序（按截止时间、推荐指数、最新发现）。
